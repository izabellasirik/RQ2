...
export const COVERAGE_TYPE_ALIASES: { match: RegExp; type: CoverageType }[] = [
  ...
  { match: /non[\s-]?trucking\s*liability|\bntl\b/i, type: 'non_trucking_liability' },
];
...
const INLINE_COVERAGE_AMOUNT =
  /^(auto\s*liability|csl|combined single limit|(?:motor truck )?cargo|physical\s*damage|warehouse(?:\s*legal)?\s*liability|general\s*liability|trailer\s*interchange|non[\s-]?trucking\s*liability)\s+\$?([\d,]+(?:\.\d+)?)\s*(?:csl)?$/i;
